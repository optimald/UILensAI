 ...
 * 
 * @param {string} [type=''] - The type of storage (screenshots, reports, comparisons)
...
async function cleanupOldFiles(verbose = false) {
  ...
  try {
    ...
    const comparisonsDir = path.join(storagePath, 'comparisons');
    try {
      await fs.mkdir(comparisonsDir, { recursive: true });
      
      // Get all comparison directories
      const comparisonDirs = await fs.readdir(comparisonsDir);
      
      for (const dir of comparisonDirs) {
        const dirPath = path.join(comparisonsDir, dir);
        const stats = await fs.stat(dirPath);
        
        if (stats.isDirectory()) {
          // Extract timestamp from the directory name (format: domain1-vs-domain2-yyyyMMdd-HHmmss-SSS)
          try {
            // Find the timestamp part of the directory name
            const matches = dir.match(/.*-(\d{8}-\d{6}-\d{3})$/);
            if (matches && matches[1]) {
              ...
              if (dirDate < cutoffDate) {
                if (verbose) {
                  console.log(`Deleting old comparison directory: ${dirPath}`);
                }
            ...
            if (verbose) {
              console.error(`Error processing comparison directory ${dir}:`, err);
            }
          }
        }
      }
    } catch (err) {
      // Comparisons directory might not exist yet, which is fine
      if (err.code !== 'ENOENT') {
        throw err;
      }
    }
