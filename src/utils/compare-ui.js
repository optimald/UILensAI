...
// Simple function to parse command line args
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (arg === '--compare-url' && i + 1 < args.length) {
      options.compareUrl = args[++i];
    }
  ...
  // Convert viewports and focus strings to arrays
  options.viewports = options.viewports.split(',');
  options.focus = options.focus.split(',');
...
function showHelp() {
  console.log(`
Usage: compare-ui --url [url] --compare-url [url] [options]

Options:
  --url              Primary URL to analyze (required)
  --compare-url      URL to compare with primary URL (required)
  --viewports        Comma-separated list of viewports (default: mobile,desktop)
  --focus            Comma-separated list of analysis areas (default: accessibility,branding,...)
  --selector         CSS selector to capture specific UI components
  --model            Model to use for analysis (default: claude-3-haiku-20240307)
  --verbose, -v      Display verbose output
  --output           Output directory for the comparison report
  --no-open          Don't open the report after completion
...
async function main() {
  try {
    ...
    if (!argv.url || !argv.compareUrl) {
      console.error(chalk.red('Error: Both --url and --compare-url are required'));
      showHelp();
      process.exit(1);
    }
