...
/**
 * Helper function to get image dimensions using the sharp library
 * 
...
async function analyzeScreenshots({ 
  ...
  // Check if ANTHROPIC_MODEL environment variable is set and override model if it is
  if (process.env.ANTHROPIC_MODEL && verbose) {
    console.log(`Using model from environment variable: ${process.env.ANTHROPIC_MODEL}`);
    model = process.env.ANTHROPIC_MODEL;
  }
  
  // Ensure maxTokens is an integer and doesn't exceed model limits
  maxTokens = parseInt(maxTokens, 10);
  const modelMaxTokens = MODEL_MAX_TOKENS[model] || 4096; // Default to 4096 if model not found
  
  if (maxTokens > modelMaxTokens) {
    console.warn(`Requested max_tokens (${maxTokens}) exceeds the maximum for ${model} (${modelMaxTokens}). Using ${modelMaxTokens} instead.`);
  ...
  if (verbose) {
    ...
    console.log(`Max tokens: ${maxTokens} (model maximum: ${modelMaxTokens})`);
  ...
  for (const [index, screenshotPath] of screenshotPaths.entries()) {
    // Calculate a unique file name/key from the screenshot path
    const fileName = path.basename(screenshotPath);
    
    console.log(`Analyzing ${fileName}...`);
    
    // Get viewport name from the file name - moved outside the try block
    const viewportNameMatch = fileName.match(/viewport-([^-]+)(?:-|--)/) || fileName.match(/viewport-([^-]+)$/);
    const viewportName = viewportNameMatch ? viewportNameMatch[1] : 'unknown';
    ...
    try {
      ...
      if (onProgress) {
        ...
        onProgress({
          step: `Analyzing ${viewportName} viewport`,
      ...
      promptText += `This screenshot shows the page at ${url} at ${viewportName} viewport size.\n\n`;
      ...
      if (verbose) {
        console.log(`Received response from Claude API for ${viewportName}`);
      ...
      if (!response.data.content || !response.data.content[0] || !response.data.content[0].text) {
        throw new Error(`Empty or invalid response from Claude API for ${viewportName}: ${JSON.stringify(response.data)}`);
      }

      analysisResults[viewportName] = {
      ...
      console.error(`Error analyzing ${viewportName} screenshot:`, error.message);
      ...
      if (error.response) {
        ...
        // Add more specific error messages for common API errors
        if (errorData && errorData.error) {
          // Handle overloaded API error
          if (errorData.error.type === 'overloaded_error') {
            analysisResults[viewportName] = {
              error: `Claude API servers are currently overloaded. Please try again in a few minutes or use a different model.`,
              screenshot: screenshotPath,
            };
          ...
          if ((errorData.error.type === 'invalid_request_error' || 
            ...
            analysisResults[viewportName] = {
              error: `Token limit exceeded: ${errorData.error.message}. Try using a smaller --max-tokens value or a different model.`,
              screenshot: screenshotPath,
            };
            continue; // Skip to the next viewport
          }
  ...
  if (Object.keys(screenshotsByViewport).length > 1 && !Object.values(analysisResults).some(r => r.error)) {
    try {
      ...
      crossPromptText += 'Analyze the responsive behavior across these viewports and identify any responsive design issues:\n\n';
      ...
      if (verbose) {
        console.log(`Sending cross-viewport analysis request using model: ${model}`);
        console.log(`Using max_tokens: ${maxTokens}`);
      }
      ...
      const crossResponse = await axios.post(
        ...
        {
          model,
          max_tokens: maxTokens, // Using the validated maxTokens value
          messages: [
            {
              role: 'user',
      ...
      // Add better error handling for common API errors
      if (error.response && error.response.data && error.response.data.error) {
        ...
        analysisResults['cross-viewport'] = {
          error: error.message || 'Unknown error during cross-viewport analysis'
        };
      }
    }
  }
...
function generatePrompt(templateData, focusAreas) {
  ...
  if (templateData.description) {
    promptText += `Important context: This page is "${templateData.description}". Please analyze the UI with this specific context in mind and reference this context in your analysis.\n\n`;
  }

  promptText += `Your feedback should be honest, critical, and constructive. Do not be overly positive or generous with ratings. Identify specific issues and be thorough in your critique while maintaining a balanced perspective. Ratings should reflect actual quality - reserve high scores (8-10) only for truly excellent implementations.\n\n`;
  
  promptText += `Focus areas for your analysis include:\n${focusAreas.join(', ')}\n\n`;
  
  promptText += `For each focus area follow this exact format:\n`;
  promptText += `- Score: [1-10]\n`;
  promptText += `- Key Observations: [list 2-3 most important observations]\n`;
  promptText += `- Critical Issues: [list specific problems in order of severity]\n`;
  promptText += `- Recommendations: [provide specific, implementable actions]\n`;
  promptText += `- Success Metrics: [1-2 measurable metrics that would indicate successful implementation]\n`;
  promptText += `- Competitive Benchmark: [how this implementation compares to industry standards]\n\n`;
  ...
  promptText += `# Overall Score\n`;
  promptText += `Provide a final score (1-10) that reflects the overall quality of the UI, weighted by the importance of each area. Explain your reasoning for this score.\n\n`;
  
  // Add implementation roadmap section
  promptText += `# Implementation Roadmap\n`;
  promptText += `Create a prioritized list of the top 5 issues to fix, ranking them by:\n`;
  promptText += `- Impact (1-5): How much improvement would fixing this create?\n`;
  promptText += `- Effort (1-5): How difficult or time-consuming is this to implement?\n`;
  promptText += `- Priority Score: Impact ÷ Effort (higher = fix first)\n\n`;
  
  promptText += `For each issue, include:\n`;
  promptText += `1. The problem description\n`;
  promptText += `2. Which focus area it belongs to\n`;
  promptText += `3. The Impact score (1-5)\n`;
  promptText += `4. The Effort score (1-5)\n`;
  promptText += `5. The Priority Score (Impact ÷ Effort)\n`;
  promptText += `6. A brief, specific recommendation for fixing it\n\n`;
  
  // Add summary section
  promptText += `# Summary\n`;
  promptText += `Provide a concise summary of the most critical issues to address first, focusing on the areas that would have the biggest impact on improving the user experience.\n\n`;
  
  promptText += `Remember to be specifically critical but fair in your assessment. Don't inflate ratings - use the full range of the scale with high scores reserved only for truly excellent implementations.`;
...
async function analyzeScreenshot(options, screenshot, templateData, focusAreas) {
  ...
  const effectiveMaxTokens = Math.min(parseInt(maxTokens, 10), modelMaxTokens);
  
  if (parseInt(maxTokens, 10) > modelMaxTokens) {
    console.warn(`Requested max_tokens (${maxTokens}) exceeds the maximum for ${model} (${modelMaxTokens}). Using ${modelMaxTokens} instead.`);
  }
  ...
  try {
    ...
    console.log(`Request payload includes model: ${model}, max_tokens: ${effectiveMaxTokens}`);
    ...
    if (error.response) {
      ...
      if (errorData && errorData.error && 
        ...
        throw new Error(`Claude API servers are currently overloaded. Please try again in a few minutes or use a different model.`);
      }
      ...
      if (errorData && errorData.error && 
         ...
         (errorData.error.message.includes('token') || 
          errorData.error.message.includes('max_tokens'))) {
        throw new Error(`Token limit exceeded: ${errorData.error.message}. Try using a smaller --max-tokens value or a different model.`);
      }
...
function parseAnalysisResponse(response) {
  ...
  try {
    ...
    // Parse JSON structure for recommendations and implementation roadmap
    const parsedData = {
    ...
    // Extract and parse recommendations JSON from the sections
    for (const sectionName in sections) {
      const sectionContent = sections[sectionName];
      // Look for recommendation JSON blocks in the content
      const jsonMatches = sectionContent.match(/```json\s*(\[.*?\])\s*```/gs);
      
      if (jsonMatches && jsonMatches.length > 0) {
        for (const match of jsonMatches) {
          try {
            // Extract the JSON content between the markdown code block delimiters
            const jsonContent = match.replace(/```json\s*|\s*```/g, '');
            
            // If this appears to be an array of recommendations
            if (jsonContent.trim().startsWith('[')) {
              const parsedRecommendations = JSON.parse(jsonContent);
              // Add the section name to each recommendation
              const enhancedRecommendations = parsedRecommendations.map(rec => ({
                ...rec,
                focusArea: sectionName
              }));
              parsedData.recommendations.push(...enhancedRecommendations);
            }
          } catch (jsonError) {
            console.error(`Error parsing JSON in ${sectionName} section:`, jsonError);
          }
        }
      }
    }
    
    // Parse implementation roadmap JSON
    if (sections['implementation roadmap']) {
      const roadmapSection = sections['implementation roadmap'];
      const roadmapMatches = roadmapSection.match(/```json\s*({.*?})\s*```/gs);
      
      if (roadmapMatches && roadmapMatches.length > 0) {
        try {
          const jsonContent = roadmapMatches[0].replace(/```json\s*|\s*```/g, '');
          parsedData.implementationRoadmap = JSON.parse(jsonContent);
        } catch (roadmapError) {
          console.error('Error parsing implementation roadmap JSON:', roadmapError);
        }
      }
    }
